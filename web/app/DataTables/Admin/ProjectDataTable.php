...
class ProjectDataTable extends DataTable
{
        ...
        return datatables()
            ...
            ->filter(function ($query) {
                ...
                if (request()->has('filter_status') and request('filter_status') <> null) {
                    $query->where('projects.status', escape_input(request('filter_status')));
                }
                if (request()->has('search') and request('search') <> null) {
                    $query->where(function ($query) {
                        $query
                            ...
                            ->orWhere('projects.id', 'like', "%" . escape_input(request('search')) . "%")
                            ->orWhere('projects.konten', 'like', "%" . escape_input(request('search')) . "%");
                    });
                }
            })
            ...
            ->addColumn('title', function ($query) {
                return '<a href="' . url('/admin/project/detail/' . $query->id . '">') . Str::limit(strip_tags($query->title), '30') . "</a>";
            })
            ...
            ->addColumn('statusx', function ($query) {
                return getstatus($query->project_status);
            })
            ->addColumn('budget', function ($query) {
                return rupiah($query->budget_from) . "-" . rupiah($query->budget_to);
            })
            ->setRowId(function ($query) {
                return $query->id;
            })
            ->rawColumns(['user_id', 'title', 'action', 'statusx']);
    }
    ...
    protected function getColumns()
    {
        return [
            ...
            ['data' => 'title', 'name' => 'postings.title', 'title' => 'TITLE'],
            ['data' => 'estimasi', 'name' => 'postings.estimasi', 'title' => 'DEADLINE'],
            ['data' => 'budget', 'name' => 'postings.budget', 'title' => 'BUDGET'],
            ['data' => 'statusx', 'name' => 'statusx', 'title' => 'STATUS'],
